...
const login = async (req, res) => {
  try {
    ...
    if (!Role) {
      ...
      return res.status(500).json({ message: "Server configuration error - Role model not loaded" });
    }
...
const register = async (req, res) => {
  ...
  try {
    ...
    if (role_id) {
      ...
      if (providedRole && (providedRole.name === 'ADMIN' || providedRole.name === 'MANAGER')) {
        return res.status(403).json({
          message: "Cannot register with ADMIN or MANAGER role. USER role will be assigned automatically.",
        });
      }
      ...
      if (!userRole) {
        try {
          ...
          try {
            ...
            console.log("Note: Permissions not yet initialized. USER role created without permissions.");
          }
        } catch (createError) {
        return res.status(500).json({
            success: false,
            message: "Failed to create USER role. Please contact administrator or call /api/setup/init",
